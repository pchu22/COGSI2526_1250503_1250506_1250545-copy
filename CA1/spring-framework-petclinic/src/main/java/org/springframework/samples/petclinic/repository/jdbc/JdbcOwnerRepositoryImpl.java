...
public class JdbcOwnerRepositoryImpl implements OwnerRepository {
    ...
    public Collection<Owner> findByLastName(String lastName) {
        List<Owner> owners = this.jdbcClient.sql("""
                SELECT id, first_name, last_name, nif, address, city, telephone
                FROM owners
    ...
    /**
     * Loads the {@link Owner} with the supplied <code>nif</code>; also loads the {@link Pet Pets} and {@link Visit Visits}
     * for the corresponding owner, if not already loaded.
     */
    @Override
    public Owner findByNif(String nif) {
        Owner owner;
        try {
            owner = this.jdbcClient.sql("""
                    SELECT id, first_name, last_name, nif, address, city, telephone
            ...
            throw new ObjectRetrievalFailureException(Owner.class, nif);
        }
        loadPetsAndVisits(owner);
        return owner;
    }
    ...
    public void save(Owner owner) {
        ...
        if (owner.isNew()) {
            ...
            this.jdbcClient.sql("""
                    UPDATE owners
                    SET first_name=:firstName, last_name=:lastName, nif=:nif, address=:address, city=:city, telephone=:telephone
                    WHERE id=:id
                    """)
